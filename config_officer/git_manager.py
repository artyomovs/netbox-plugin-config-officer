...
def get_file_repo_state(repository_path, filename):
    ...
    file_commits = list(
        RepositoryMining(
            repository_path, only_commits=file_commits_list
        ).traverse_commits()
    )
    ...
    if repo_state["commits_count"] > 0:
        repo_state["last_commit_date"] = file_commits[0].author_date.strftime(
            "%d %b %Y %H:%M"
        )
        repo_state["first_commit_date"] = file_commits[-1].author_date.strftime(
            "%d %b %Y %H:%M"
        )
        repo_state["commits"] = []
